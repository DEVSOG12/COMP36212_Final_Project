...
void initialise_weight_matrices(void){
    ...
    for (int i=0; i < N_NEURONS_LI; i++){
        for (int j=0; j <N_NEURONS_L1; j++){
            ...
            w_LI_L1[i][j].dw = 0.0;
            w_LI_L1[i][j].v = 0.0;  // Initialize velocity to zero
            w_LI_L1[i][j].m = 0.0;  // Initialize Adam first moment to zero
            w_LI_L1[i][j].v_adam = 0.0;  // Initialize Adam second moment to zero
        }
    }
    ...
    for (int i=0; i < N_NEURONS_L1; i++){
        for (int j=0; j <N_NEURONS_L2; j++){
            ...
            w_L1_L2[i][j].dw = 0.0;
            w_L1_L2[i][j].v = 0.0;  // Initialize velocity to zero
            w_L1_L2[i][j].m = 0.0;  // Initialize Adam first moment to zero
            w_L1_L2[i][j].v_adam = 0.0;  // Initialize Adam second moment to zero
        }
    }
    ...
    for (int i=0; i < N_NEURONS_L2; i++){
        for (int j=0; j <N_NEURONS_L3; j++){
            ...
            w_L2_L3[i][j].dw = 0.0;
            w_L2_L3[i][j].v = 0.0;  // Initialize velocity to zero
            w_L2_L3[i][j].m = 0.0;  // Initialize Adam first moment to zero
            w_L2_L3[i][j].v_adam = 0.0;  // Initialize Adam second moment to zero
        }
    }
    ...
    for (int i=0; i < N_NEURONS_L3; i++){
        for (int j=0; j <N_NEURONS_LO; j++){
            ...
            w_L3_LO[i][j].dw = 0.0;
            w_L3_LO[i][j].v = 0.0;  // Initialize velocity to zero
            w_L3_LO[i][j].m = 0.0;  // Initialize Adam first moment to zero
            w_L3_LO[i][j].v_adam = 0.0;  // Initialize Adam second moment to zero
        }
    }
}
...
double verify_gradients(unsigned int sample_idx) {
    printf("Verifying gradients for sample %u using numerical differentiation\n", sample_idx);
    ...
    // Store the original weight to restore it later
    ...
    evaluate_forward_pass(training_data, sample_idx);
    double loss = compute_xent_loss(training_labels[sample_idx]);
    evaluate_backward_pass_sparse(training_labels[sample_idx], sample_idx);
    store_gradient_contributions();
    
    // Sample a small subset of weights for verification (checking all would be too slow)
    int num_weights_to_check = 10; // Per layer
    
    // Check selected weights in w_L3_LO
    for (int check = 0; check < num_weights_to_check; check++) {
        int i = rand() % N_NEURONS_L3;
        int j = rand() % N_NEURONS_LO;
        
        // Save original weight
        original_weight = w_L3_LO[i][j].w;
        
        // Forward pass with w + epsilon
        w_L3_LO[i][j].w = original_weight + epsilon;
        evaluate_forward_pass(training_data, sample_idx);
        double loss_plus = compute_xent_loss(training_labels[sample_idx]);
        
        // Forward pass with w - epsilon
        w_L3_LO[i][j].w = original_weight - epsilon;
        evaluate_forward_pass(training_data, sample_idx);
        double loss_minus = compute_xent_loss(training_labels[sample_idx]);
        ...
        double analytical_gradient = w_L3_LO[i][j].dw;
        double abs_error = fabs(numerical_gradient - analytical_gradient);
        double relative_error = abs_error / (fabs(analytical_gradient) + 1e-10);
        ...
        if (check == 0) {
            printf("Sample Layer3->Output[%d][%d]: Analytical: %.6f, Numerical: %.6f, RelErr: %.6f\n", 
                i, j, analytical_gradient, numerical_gradient, relative_error);
        }
    }
    
    // Check selected weights in w_L2_L3
    for (int check = 0; check < num_weights_to_check; check++) {
        int i = rand() % N_NEURONS_L2;
        int j = rand() % N_NEURONS_L3;
        
        original_weight = w_L2_L3[i][j].w;
        
        w_L2_L3[i][j].w = original_weight + epsilon;
        evaluate_forward_pass(training_data, sample_idx);
        double loss_plus = compute_xent_loss(training_labels[sample_idx]);
        ...
        w_L2_L3[i][j].w = original_weight;
        
        double numerical_gradient = (loss_plus - loss_minus) / (2.0 * epsilon);
        double analytical_gradient = w_L2_L3[i][j].dw;
        double abs_error = fabs(numerical_gradient - analytical_gradient);
        double relative_error = abs_error / (fabs(analytical_gradient) + 1e-10);
        ...
        if (check == 0) {
            printf("Sample Layer2->Layer3[%d][%d]: Analytical: %.6f, Numerical: %.6f, RelErr: %.6f\n", 
                i, j, analytical_gradient, numerical_gradient, relative_error);
        }
    }
    
    // Check selected weights in w_L1_L2
    for (int check = 0; check < num_weights_to_check; check++) {
        int i = rand() % N_NEURONS_L1;
        int j = rand() % N_NEURONS_L2;
        
        original_weight = w_L1_L2[i][j].w;
        
        w_L1_L2[i][j].w = original_weight + epsilon;
        evaluate_forward_pass(training_data, sample_idx);
        double loss_plus = compute_xent_loss(training_labels[sample_idx]);
        ...
        w_L1_L2[i][j].w = original_weight;
        
        double numerical_gradient = (loss_plus - loss_minus) / (2.0 * epsilon);
        double analytical_gradient = w_L1_L2[i][j].dw;
        double abs_error = fabs(numerical_gradient - analytical_gradient);
        double relative_error = abs_error / (fabs(analytical_gradient) + 1e-10);
        ...
        if (check == 0) {
            printf("Sample Layer1->Layer2[%d][%d]: Analytical: %.6f, Numerical: %.6f, RelErr: %.6f\n", 
                i, j, analytical_gradient, numerical_gradient, relative_error);
        }
    }
    
    // Check selected weights in w_LI_L1
    for (int check = 0; check < num_weights_to_check; check++) {
        int i = rand() % N_NEURONS_LI;
        int j = rand() % N_NEURONS_L1;
        
        original_weight = w_LI_L1[i][j].w;
        
        w_LI_L1[i][j].w = original_weight + epsilon;
        evaluate_forward_pass(training_data, sample_idx);
        double loss_plus = compute_xent_loss(training_labels[sample_idx]);
        ...
        w_LI_L1[i][j].w = original_weight;
        
        double numerical_gradient = (loss_plus - loss_minus) / (2.0 * epsilon);
        double analytical_gradient = w_LI_L1[i][j].dw;
        double abs_error = fabs(numerical_gradient - analytical_gradient);
        double relative_error = abs_error / (fabs(analytical_gradient) + 1e-10);
        ...
        if (check == 0) {
            printf("Sample Input->Layer1[%d][%d]: Analytical: %.6f, Numerical: %.6f, RelErr: %.6f\n", 
                i, j, analytical_gradient, numerical_gradient, relative_error);
        }
    }
    
    double avg_relative_error = total_relative_error / num_checks;
